      ...
      allowTouchMove: (el) => {
        while (el && el !== document.body) {
          if (
            ...
            el.getAttribute("data-popout-list") !== null ||
            el.tagName === "TEXTAREA"
          ) {
            return true;
          }
    ...
    document.addEventListener(
      ...
      (e) => {
        if (
          ...
          e.target.classList.contains(classes$O.loading)
        ) {
  ...
  class PredictiveSearch extends HeaderSearchForm {
    ...
    onFormSubmit(event) {
      if (
        ...
        this.querySelector(selectors$X.selectedLink)
      )
        event.preventDefault();
    }
    ...
    switchOption(direction) {
      ...
      const allVisibleElements = Array.from(
        this.querySelectorAll(selectors$X.allVisibleElements)
      ...
      fetch(
        ...
        .then((text) => {
          const resultsMarkup = new DOMParser()
            ...
            .querySelector(selectors$X.sectionPredictiveSearch).innerHTML;
    ...
    setLiveRegionLoadingState() {
      this.statusElement =
        this.statusElement ||
        this.querySelector(selectors$X.predictiveSearchStatus);
      this.loadingText =
    ...
    setLiveRegionResults() {
      ...
      this.setLiveRegionText(
        this.querySelector(selectors$X.searchResultsLiveRegion).textContent
      );
    }
  ...
  */
  ...
  Shopify.Cart.ShippingCalculator = (function () {
    ...
    const _render = function (response) {
      ...
      if (template && wrapper) {
        ...
        if (response.success) {
          ...
          const noShippingElem = createdNewElem.querySelector(
            "[data-template-no-shipping]"
          );

          if (response.rates.length < 1 && noShippingElem) {
            ratesText = noShippingElem.getAttribute(
              "data-template-no-shipping"
            );
          }
        } else {
    ...
    const _onError = function (XMLHttpRequest) {
      ...
      if (shippingChild) {
        while (shippingChild.firstChild)
    ...
    const _onCartShippingRatesUpdate = function (rates, shipping_address) {
      ...
      if (rates.length && shippingChild) {
        shippingChild.textContent =
          rates[0].price == "0.00"
            ? window.theme.strings.free
            : themeCurrency.formatMoney(rates[0].price, theme.moneyFormat);
      }
      _render({
        rates: rates,
    ...
    const _init = function () {
      ...
      if (getRatesButton) {
        getRatesButton.addEventListener("click", function (e) {
          ...
          const elemCountryData = selectCountry.getAttribute(
          ...
          const elemProvinceData = selectProvince.getAttribute(
            "data-default-fullname"
          );
          if (
            elemProvinceVal === "" &&
    ...
    changeValueOnClick() {
      ...
      this.buttons.forEach((element) => {
        element.addEventListener("click", (event) => {
          ...
          const isDescrease =
            clickedElement.matches(that.settings.quantityMinusButton) ||
            clickedElement.closest(that.settings.quantityMinusButton);
          const isIncrease =
            clickedElement.matches(that.settings.quantityPlusButton) ||
            clickedElement.closest(that.settings.quantityPlusButton);
  ...
  class CartDrawer {
    ...
    init() {
      ...
      this.cartOriginalTotal = document.querySelector(
        selectors$U.cartOriginalTotal
      );
      this.cartOriginaTotalPrice = document.querySelector(
        selectors$U.cartOriginaTotalPrice
      );
      this.cartDiscountHolder = document.querySelector(
        selectors$U.cartDiscountsHolder
      );
      ...
      this.upsellProductsHolder = document.querySelector(
        selectors$U.upsellProductsHolder
      );
      ...
      this.requestItemsAnimationFrame =
        this.requestItemsAnimationFrame.bind(this);
          ...
          if (widgets.length > 1) {
            widgets.forEach((content) => {
              if (content !== widget.parentElement) {
                const buttonExpand = content.querySelector(
                  selectors$U.expandButton
                );
    ...
    cartEventAdd() {
      document.addEventListener("click", (event) => {
        ...
        if (
          clickedElement.matches(selectors$U.buttonAddToCart) ||
          (clickedElement.closest(selectors$U.buttonAddToCart) &&
            clickedElement)
        ) {
          let formData = "";
          let button = clickedElement.matches(selectors$U.buttonAddToCart)
            ? clickedElement
            : clickedElement.closest(selectors$U.buttonAddToCart);
    ...
    cartEventRemoveError() {
      document.addEventListener("click", (event) => {
        ...
        if (
          clickedElement.matches(selectors$U.formCloseError) ||
          clickedElement.closest(selectors$U.formCloseError)
        ) {
          event.preventDefault();

          const errorContainer = clickedElement.closest(
            selectors$U.formErrorsContainer
          );
    ...
    addToCart(formData, quickAddObject = null, button = null) {
      ...
      if (this.cartDrawerEnabled) {
        ...
        if (quickAddHolder) {
          ...
          buttonQuickAddMobile = quickAddHolder.querySelector(
            selectors$U.buttonQuickAddMobile
          );
        }
      }
    ...
    disableCartButtons() {
      ...
      const buttons = this.cart.querySelectorAll(
        `button, ${selectors$U.cartItemRemove}`
      );
    ...
    updateErrorText(itemTitle) {
      this.cartErrorHolder.querySelector(selectors$U.errorMessage).innerText =
        itemTitle;
    }
    ...
    addToCartError(data, quickAddHolder, button) {
      if (
        ...
        button &&
        button.closest(selectors$U.cartDrawer) !== null &&
        !button.closest(selectors$U.cartDrawer)
      ) {
      ...
      if (button !== null) {
        const outerContainer =
          button.closest(selectors$U.outerSection) ||
          button.closest(selectors$U.quickAddHolder) ||
          button.closest(selectors$U.quickAddModal);
        let errorContainer = outerContainer?.querySelector(
          selectors$U.formErrorsContainer
        );
        ...
        if (
          buttonUpsellHolder &&
          buttonUpsellHolder.querySelector(selectors$U.formErrorsContainer)
        ) {
    ...
    closeCartDrawer() {
      ...
      this.cartErrorHolder.classList.remove(classes$L.expanded);
      this.cartDrawer
        .querySelectorAll(selectors$U.animation)
        .forEach((item) => {
      ...
      if (this.body.classList.contains(classes$L.focused)) {
        const buttonOpenCart = this.headerWrapper.querySelectorAll(
          `${selectors$U.cartToggleElement}`
        );
    ...
    cartToggleEvents() {
      document.addEventListener("click", (event) => {
        const clickedElement = event.target;
        const isNotCartButton = !(
          clickedElement.matches(selectors$U.cartToggleElement) ||
          clickedElement.closest(selectors$U.cartToggleElement)
        );
        const isNotCartDrawerUnderlay = !(
          clickedElement.matches(selectors$U.drawerUnderlay) ||
          clickedElement.closest(selectors$U.drawerUnderlay)
        );
        const isNotCartDrawerOrCartDrawerChild =
          !(
            clickedElement.matches(selectors$U.cartDrawer) ||
            clickedElement.closest(selectors$U.cartDrawer)
          ) || !isNotCartDrawerUnderlay;
        const isNotPairProduct = !(
          clickedElement.matches(selectors$U.buttonSkipUpsellProduct) ||
          clickedElement.closest(selectors$U.buttonSkipUpsellProduct)
        );
        ...
        if (
          ...
          isNotCartButton &&
          isNotCartDrawerOrCartDrawerChild &&
          isNotPairProduct
        ) {
          this.closeCartDrawer();
        }
    ...
    buildTotalPrice(data) {
      if (
        ...
        this.cartOriginaTotalPrice.innerHTML =
    ...
    buildCartTotalDiscounts(discounts) {
      ...
      discounts.forEach((discount) => {
        ...
        discountTemplate.querySelector(
          selectors$U.cartTotalDiscountTitle
        ).innerHTML = discount.title;
        discountTemplate.querySelector(
          selectors$U.cartTotalDiscountAmount
    ...
    freeShippingMessageHandle(total) {
      ...
      this.cartFreeShippingLimit =
        Number(
          this.freeShipping[0].getAttribute(attributes$j.freeShippingLimit)
        ) *
        100 *
      ...
      this.freeShipping.forEach((message) => {
        const hasQualifiedShippingMessage =
          message.hasAttribute(attributes$j.freeShipping) &&
          message.getAttribute(attributes$j.freeShipping) === "true" &&
          total >= 0;
    ...
    updateProgress() {
      ...
      const percentValue = isNaN(this.subtotal / this.cartFreeShippingLimit)
        ? 100
      ...
      this.freeShipping.forEach((item) => {
        const progressBar = item.querySelector(
          selectors$U.freeShippingProgress
        );
    ...
    skipUpsellProductEvent() {
      ...
      const skipButtons = this.upsellProductsHolder.querySelectorAll(
        selectors$U.buttonSkipUpsellProduct
      );
        ...
        skipButtons.forEach((button) => {
          button.addEventListener("click", (event) => {
            ...
            const productID = button
              .closest(selectors$U.quickAddHolder)
              .getAttribute(attributes$j.quickAddHolder);
    ...
    checkSkippedUpsellProductsFromStorage() {
      ...
      const skippedUpsellItemsFromStorageArray =
        skippedUpsellItemsFromStorage.split(",");
    ...
    removeUpsellProduct(productID) {
      ...
      const upsellProduct = this.upsellProductsHolder.querySelector(
        `[${attributes$j.quickAddHolder}="${productID}"]`
      );
    ...
    toggleCartUpsellWidgetVisibility() {
      ...
      const upsellItems = this.upsellProductsHolder.querySelectorAll(
        selectors$U.quickAddHolder
      );
      const cartWidget = this.upsellProductsHolder.closest(
        selectors$U.cartWidget
      );

      if (!cartWidget) return;

      const cartWidgetToggleButton = cartWidget.querySelector(
        selectors$U.expandButton
      );
      const cartWidgetContent = cartWidget.querySelector(
        selectors$U.cartWidgetContent
      );
      ...
      if (cartWidgetToggleButton) {
        cartWidgetToggleButton.classList.toggle(
          classes$L.active,
          upsellItems.length
        );
      }
      ...
      if (cartWidgetContent) {
        cartWidgetContent.classList.toggle(
          classes$L.expanded,
          upsellItems.length
        );
      }
    ...
    observeAdditionalCheckoutButtons() {
      ...
      const additionalCheckoutButtons = this.cartDrawer.querySelector(
        selectors$U.additionalCheckoutButtons
      );
  ...
  class Accessibility {
    ...
    init() {
      ...
      this.linkesWithOnlyHash = document.querySelectorAll(
        selectors$T.linkesWithOnlyHash
      );
    ...
    focusEvents() {
      ...
      document.addEventListener("keyup", (event) => {
        ...
        const pressEnterOrSpace =
          event.code === window.theme.keyboardKeys.ENTER ||
          event.code === window.theme.keyboardKeys.SPACE;
        const targetElement =
          target.matches(selectors$T.triggerFocusElement) ||
          target.closest(selectors$T.triggerFocusElement);
        const isAccordion =
        ...
        const isTab =
          target.classList.contains(classes$J.tabLink) ||
          target.parentNode.classList.contains(classes$J.tabLink);
  ...
  class SearchPopdown extends HTMLElement {
    ...
    onPopdownToggleClick(event) {
      event.preventDefault();
      event.target.closest(selectors$R.details).hasAttribute("open")
        ? this.close()
        : this.open(event);
    }
    ...
    onBodyClick(event) {
      if (
        ...
        event.target.hasAttribute(attributes$i.popdownUnderlay)
      )
        this.close();
    }
  ...
  theme.ProductModel = (function () {
    ...
    function pauseOtherMedia(mediaId) {
      ...
      const currentMedia = document.querySelector(
        `${selectors.productMediaWrapper}${mediaIdString}`
      );
      const otherMedia = document.querySelectorAll(
        `${selectors.productMediaWrapper}:not(${mediaIdString})`
      );
  ...
  class Addresses {
          ...
          editButtons.forEach((element) => {
            element.addEventListener("click", function () {
              ...
              section
                ...
                .classList.toggle(classes$I.hidden);
            });
          });
        }
    ...
    customerAddresses() {
      ...
      const countryOptions = this.section.querySelectorAll(
        selectors$Q.addressCountryOption
      );
    ...
    addPin(stopMouseEnter = false) {
      ...
      if (
        ...
        const tooltipTargetWindowDifference =
          tooltipLeftWithWidth - getWindowWidth();
        ...
        tooltipTargetArrow.style.left = `${
          tooltipRect.left + tooltipWidth / 2
        }px`;
  ...
  const slideDown = (
    ...
    if (
      ...
      target.classList.contains(classes$G.sliding)
    )
      return;
  ...
  class GlobalAccordions {
    ...
    accordionEvents() {
      this.accordionToggles.forEach((element) => {
        element.addEventListener("click", (event) => {
          event.preventDefault();
          const targetAccordionBody = element.parentElement.querySelector(
            selectors$L.accordionBody
          );
          if (
            targetAccordionBody &&
            !targetAccordionBody.classList.contains(classes$F.sliding)
          ) {
      ...
      if (
        this.accordion.getAttribute(attributes$h.accordionExpandValue) ===
        "true"
      ) {
    ...
    closeOtherAccordions(element, slide = true) {
      ...
      if (holder) {
        otherElements = [
          ...holder.querySelectorAll(selectors$L.accordionToggle),
        ];
      }

      otherElements.filter((otherElement) => {
        const otherElementAccordionBody =
          otherElement.parentElement.querySelector(selectors$L.accordionBody);
        if (
          otherElement !== element &&
          otherElement.classList.contains(classes$F.open) &&
          otherElementAccordionBody
        ) {
    ...
    onAccordionToggle(element, body) {
      ...
      element.ariaExpanded = element.classList.contains(classes$F.open)
        ? "true"
        : "false";
    ...
    onBlockToggle(evt, blockSelect = true) {
      const targetAccordionToggle = this.container.querySelector(
        `${selectors$L.accordionToggle}[${attributes$h.accordionBlockValue}="${evt.detail.blockId}"]`
      );
      if (!targetAccordionToggle) return;
      const targetAccordionBody =
        targetAccordionToggle.parentElement.querySelector(
          selectors$L.accordionBody
        );
  ...
  class Slider {
    constructor(container, slideshow = null) {
      ...
      this.slideshow =
        slideshow || this.container.querySelector(selectors$K.slider);
      ...
      this.sliderThumbs = this.container.querySelectorAll(
        selectors$K.sliderThumb
      );
      this.multipleSlides = this.slideshow.hasAttribute(
        attributes$g.slidesLargeDesktop
      );
      this.isHeightEqualized =
        this.slideshow.getAttribute(attributes$g.equalizeHeight) === "true";
    ...
    addRemoveSlidesForDevices() {
      this.hasDiffSlidesForMobileDesktop =
        Array.prototype.filter.call(this.slideshowSlides, (slide) => {
          if (
            slide.classList.contains(classes$E.desktop) ||
            slide.classList.contains(classes$E.mobile)
          ) {
            return slide;
          }
        }).length > 0;
    ...
    slideActions(changeEvent = false) {
      const currentSlide = this.slideshow.querySelector(
        `.${classes$E.isSelected}`
      );
      const currentSlideTextColor = currentSlide.getAttribute(
        attributes$g.slideTextColor
      );
      const currentSlideLink = currentSlide.querySelector(selectors$K.link);
      const buttons = this.slideshow.querySelectorAll(
        `${selectors$K.slide} a, ${selectors$K.slide} button`
      );
      ...
      if (buttons.length) {
        buttons.forEach((button) => {
          ...
          if (slide) {
            const tabIndex = slide.classList.contains(classes$E.isSelected)
              ? 0
              : -1;
      ...
      if (
        currentSlideTextColor !== "rgba(0,0,0,0)" &&
      ...
      if (
        ...
        currentSlide.hasAttribute(attributes$g.slideIndex)
      ) {
        const slideIndex = parseInt(
          currentSlide.getAttribute(attributes$g.slideIndex)
        );
        const currentThumb = this.container.querySelector(
          `${selectors$K.sliderThumb}.${classes$E.isSelected}`
        );
        if (currentThumb) {
    ...
    positionArrows() {
      if (
        this.slideshow.hasAttribute(attributes$g.arrowPositionMiddle) &&
        this.sliderOptions.prevNextButtons
      ) {
        const itemImage =
          this.slideshow.querySelector(selectors$K.collectionImage) ||
          this.slideshow.querySelector(selectors$K.productItemImage) ||
          this.slideshow.querySelector(selectors$K.columnImage);

        // Prevent 'clientHeight' of null error if no image
        if (!itemImage) return;

        this.slideshow.querySelector(selectors$K.flickityPrevArrow).style.top =
          itemImage.clientHeight / 2 + "px";
        this.slideshow.querySelector(selectors$K.flickityNextArrow).style.top =
          itemImage.clientHeight / 2 + "px";
      }
    }
    ...
    onBlockSelect(evt) {
      ...
      const slide = this.slideshow.querySelector(
        `[${selectors$K.slideValue}="${evt.detail.blockId}"]`
      );
      ...
      if (
        this.multipleSlides &&
        !this.slideshow.classList.contains(classes$E.sliderInitialized)
      ) {
        slideIndex = 0;
      }
      ...
      if (
        this.flkty &&
        this.slideshow.classList.contains(classes$E.flickityEnabled)
      ) {
  ...
  class Popout {
    ...
    popupToggleClick(evt) {
      ...
      const ariaExpanded =
        button.getAttribute(selectors$J.ariaExpanded) === "true";

      if (this.productGrid) {
        const productGridItemImage = this.productGrid.querySelector(
          selectors$J.productGridImage
        );
    ...
    popupOptionsClick(evt) {
      ...
      if (link.attributes.href.value === "#") {
        ...
        if (this.popoutPrevent) {
          ...
          if (
            this.popoutInput.hasAttribute(selectors$J.popoutQuantity) &&
          ...
          if (
            targetAttribute &&
            targetAttribute.hasAttribute(`${selectors$J.ariaCurrent}`)
          ) {
          ...
          if (attrValue !== "") {
            ...
            if (
              this.popoutToggleText.hasAttribute(
                selectors$J.popoutToggleTextValue
              ) &&
  ...
  const scrollTo = (elementTop) => {
    ...
    const headerHeight =
      ...
      document.querySelector(selectors$I.headerHeight)
        ? document
            .querySelector(selectors$I.headerHeight)
    ...
    constructor(newsletter) {
      ...
      if (this.popup) {
        this.cookie = new PopupCookie(
          this.popup.getAttribute(attributes$f.cookieNameAttribute),
          "user_has_closed",
          null
        );
      }
  ...
  class ProductForm {
    constructor(element, product, options) {
      ...
      this.planInputs = this._initInputs(
        selectors$G.planInput,
        options.onPlanChange
      );

      this.quantityInputs = this._initInputs(
        selectors$G.quantityInput,
        options.onQuantityChange
      );

      this.propertyInputs = this._initInputs(
        selectors$G.propertyInput,
  ...
  class NativeScrollbar {
    constructor(scrollbar) {
      ...
      this.arrowPrev = this.scrollbar.parentNode.querySelector(
        selectors$F.scrollbarArrowPrev
      );
    ...
    goToNext() {
      const moveWith = this.scrollbar.hasAttribute(
        selectors$F.scrollbarSlideFullWidth
      )
  ...
  class Swatch {
    ...
    handleEvents() {
      ...
      if (this.outer) {
        ...
        this.imagesHidden = this.outer.querySelectorAll(
          `[${selectors$E.variantTitle}][style*="display: none;"]`
        );
    ...
    changeImage() {
      if (this.image) {
        ...
        const imageTarget = this.slide.querySelector(
          `[${selectors$E.variantTitle}="${variantName}"]`
        );

        if (imageTarget) {
          const imageVisible = this.slide.querySelector(
            `[${selectors$E.variantTitle}].${classes$A.visible}`
          );
          if (imageVisible) {
    ...
    init() {
      ...
      this.swatches.forEach((swatch) => {
        ...
        if (variant) {
          ...
          const swatchLink = swatchTemplate.querySelector(
            selectors$E.swatchLink
          );
  ...
  const makeGridSwatches = (section) => {
    const gridSwatchWrappers = section.container.querySelectorAll(
      selectors$E.wrapper
    );
    ...
    constructor(container) {
      ...
      if (this.player) {
        ...
        this.enableBackground =
          this.player.getAttribute(selectors$D.dataEnableBackground) === "true";
        this.disableSound =
          this.player.getAttribute(selectors$D.dataEnableSound) === "false";
        this.enableAutoplay =
          this.player.getAttribute(selectors$D.dataEnableAutoplay) !== "false";
        this.enableLoop =
          this.player.getAttribute(selectors$D.dataEnableLoop) !== "false";
  ...
  class LoadVideoYT {
    constructor(container) {
      ...
      if (this.player) {
        ...
        if (this.videoType == "youtube") {
          this.playerID = this.player.querySelector(selectors$C.youtubeWrapper)
            ? this.player.querySelector(selectors$C.youtubeWrapper).id
            : this.player.id;
          ...
          this.container.addEventListener(
            ...
            function (e) {
              if (
                e.target.matches(selectors$C.videoWrapper) ||
                e.target.closest(selectors$C.videoWrapper)
              ) {
                const playerID = e.target.querySelector(
                  selectors$C.videoIframe
                ).id;
        ...
        events: {
          onReady: (event) => {
            ...
            const enableSound =
              document
                .querySelector(`#${id}`)
                .getAttribute(selectors$C.dataEnableSound) === "true";
          ...
          onStateChange: (event) => {
            ...
            if (event.data == 1) {
              ...
              event.target
                .getIframe()
                .parentElement.classList.add(classes$y.loaded);
            }
          },
        },
      };
    ...
    onUnload() {
      const playerID =
        "youtube-" + this.container.getAttribute(selectors$C.dataSectionId);
  ...
  class LoadPhotoswipe {
    ...
    initLoader() {
      if (
        this.pswpElement.classList.contains(classes$x.customLoader) &&
        this.options !== "" &&
        this.options.mainClass
      ) {
        this.pswpElement.setAttribute(
          attributes$e.dataOptionClasses,
          this.options.mainClass
        );
    ...
    loadPopup() {
      ...
      if (
        ...
        this.options.closeElClasses.forEach((closeClass) => {
          const closeElement = this.pswpElement.querySelector(
            `.pswp__${closeClass}`
          );
          if (closeElement) {
    ...
    initVideo() {
      const videoContainer = this.pswpElement.querySelector(
        selectors$B.popupCustomIframe
      );
      if (videoContainer) {
        const videoType = videoContainer.getAttribute(
          attributes$e.dataVideoType
        );
        this.isVideo = true;
    ...
    thumbsActions() {
      if (this.popupThumbsContainer && this.popupThumbsContainer.firstChild) {
        ...
        this.popupThumbs.forEach((element, i) => {
          element.addEventListener("click", (e) => {
            e.preventDefault();
            const lastCurrentElement = element.parentElement.querySelector(
              `.${classes$x.current}`
            );
    ...
    onClose() {
      ...
      if (
        ...
        document.body.classList.contains(classes$x.isFocused)
      ) {
        requestAnimationFrame(() => {
    ...
    setCurrentThumb() {
      const lastCurrentThumb = this.pswpElement.querySelector(
        `${selectors$B.popupThumbs} > .${classes$x.current}`
      );
      if (lastCurrentThumb) {
  ...
  class Zoom {
    constructor(section) {
      ...
      this.zoomEnable =
        this.container.getAttribute(selectors$A.dataImageZoomEnable) === "true";
    ...
    createZoom(indexImage) {
      ...
      this.zoomWrappers.forEach((elementImage) => {
        ...
        const imgAlt = elementImage.hasAttribute(selectors$A.dataImageAlt)
          ? elementImage.getAttribute(selectors$A.dataImageAlt)
          : "";
        ...
        if (this.zoomWrappers.length === counter) {
          const options = {
            ...
            mainClass:
              counter === 1
                ? `${classes$w.popupClass} ${classes$w.popupClassNoThumbs}`
                : `${classes$w.popupClass}`,
    ...
    zoomCaption(item, captionEl) {
      ...
      captionHtml = captionHtml.replaceAll(
        classes$w.popupTitle,
        classes$w.popupTitleNew
      );
  ...
  class Video {
    constructor(section) {
      ...
      this.id = section.id;
      this.tallLayout =
        this.container.getAttribute(selectors$z.dataTallLayout) === "true";
      this.players = {};
      this.init();
    }
    ...
    pauseOtherMedia(mediaId) {
      ...
      const otherMedia = document.querySelectorAll(
        `${selectors$z.productMediaWrapper}:not(${mediaIdString})`
      );

      document
        .querySelector(`${selectors$z.productMediaWrapper}${mediaIdString}`)
        .classList.remove(classes$v.mediaHidden);
    ...
    launch3d() {
      document.addEventListener("shopify_xr_launch", () => {
        const currentMedia = this.container.querySelector(
          `${selectors$y.modelViewer}:not(.${classes$u.mediaHidden})`
        );
  ...
  class InitSlider {
    constructor(section, modalHolder = null) {
      ...
      this.tallLayout =
        this.container.getAttribute(selectors$x.dataTallLayout) === "true";
      this.slideshow = this.container.querySelector(
        selectors$x.productSlideshow
      );
      ...
      if (
        this.slideshow &&
        this.slideshow.hasAttribute(attributes$d.sliderOptions)
      ) {
        this.customOptions = JSON.parse(
          decodeURIComponent(
            this.slideshow.getAttribute(attributes$d.sliderOptions)
          )
        );
      }
    ...
    destroySlider() {
      const isSliderInitialized = this.slideshow.classList.contains(
        classes$t.sliderEnabled
      );
    ...
    createSlider() {
      ...
      const instance = this;
      const firstSlide = this.slideshow.querySelectorAll(
        `[${selectors$x.mediaType}]`
      )[0];
      let options = {
        ...sliderOptions,
        on: {
            ...
            slides.addEventListener("keyup", (e) => {
              if (e.code === window.theme.keyboardKeys.ENTER) {
                const zoomElement = slides.querySelector(
                  `.${classes$t.selected} ${selectors$x.zoomElement}`
                );
                if (zoomElement) {
                  zoomElement.dispatchEvent(
      ...
      this.flkty.on("change", function (index) {
        ...
        if (instance.thumbs) {
          const selectedElem = instance.thumbs.querySelector(
            selectors$x.classSelected
          );
          const currentSlide = instance.thumbs.querySelector(
            `${selectors$x.sliderThumb} [${selectors$x.dataThumbIndex}="${index}"]`
          );

          if (selectedElem) {
            const selectedElemThumb = selectedElem.querySelector(
              `[${selectors$x.dataThumbIndex}]`
            );
      ...
      this.flkty.on("settle", function () {
        ...
        if (deferredMedia && deferredMedia.getAttribute("loaded") !== true) {
          currentMedia
            .querySelector(selectors$x.deferredMediaButton)
      ...
      this.flkty.on("dragEnd", (event, pointer) => {
        const draggedElem = this.flkty.element.querySelector(
          `.${classes$t.dragging}`
        );
        if (draggedElem) {
      ...
      this.container.addEventListener("click", (e) => {
        ...
        if (
          target.matches(selectors$x.productSlideThumb) ||
          target.closest(selectors$x.productSlideThumb)
        ) {
          e.preventDefault();
          const selector = target.matches(selectors$x.productSlideThumb)
            ? target
            : target.closest(selectors$x.productSlideThumb);
          const slideIdx = selector.hasAttribute(selectors$x.dataThumbIndex)
            ? parseInt(selector.getAttribute(selectors$x.dataThumbIndex))
            : 0;
    ...
    accessibilityActions(slider) {
      ...
      if (slides.length) {
        slides.forEach((element) => {
          ...
          const focusedElements = slide.querySelectorAll(
            `model-viewer, video, iframe, button, [href], input, ${selectors$x.focusedElement}`
          );
      ...
      if (this.thumbs && slides.length) {
        ...
        slides.forEach((element, i) => {
          ...
          if (slide.querySelector(`[${selectors$x.ariaLabel}]`)) {
            thumbAlt = slide
              .querySelector(`[${selectors$x.ariaLabel}]`)
              .getAttribute(selectors$x.ariaLabel);
          }
  ...
  class SelloutVariants {
    ...
    getCurrentState() {
      ...
      const options = this.container.querySelectorAll(
        selectors$w.optionInputCurrent
      );
      if (options.length) {
  ...
  class ProductAddForm {
    constructor(section, modalHolder = null) {
      ...
      this.container = modalHolder || section.container;
      this.tallLayout =
        this.container.getAttribute(attributes$c.dataTallLayout) === "true";
      ...
      this.enableHistoryState =
        this.container.getAttribute(attributes$c.dataEnableHistoryState) ===
        "true";
      this.hasUnitPricing = this.container.querySelector(
        selectors$v.unitWrapper
      );
      this.subSelectors = this.container.querySelector(
        selectors$v.subSelectors
      );
          ...
          if (variant) {
            if (variant.available) {
              element.classList.remove(
                classes$r.variantSoldOut,
                classes$r.variantUnavailable
              );
            } else {
            ...
            const formSelect = element.querySelector(
              selectors$v.originalSelectorId
            );
            if (formSelect) {
    ...
    updateRemaining(formState) {
      ...
      this.remainingWrapper?.classList.remove(
        classes$r.remainingIn,
        classes$r.remainingOut,
        classes$r.remainingUnavailable,
        classes$r.remainingLow
      );
        ...
        toggle.addEventListener(
          ...
          function (e) {
            ...
            const selected = this.container.querySelector(
              `[${selectors$v.subsChild}="${val}"]`
            );
            const groups = this.container.querySelectorAll(
              `[${selectors$v.subsChild}]`
            );
            if (selected) {
    ...
    updateProductUnits(formState) {
      ...
      if (unitPrice) {
        ...
        const formattedPrice = themeCurrency.formatMoney(
          unitPrice,
          theme.moneyFormat
        );
        this.container.querySelector(selectors$v.unitPrice).innerHTML =
          formattedPrice;
      ...
      if (variant) {
        ...
        if (variant.featured_media) {
          const newImg = this.container.querySelector(
            `[${attributes$c.dataImageId}="${variant.featured_media.id}"]`
          );
          ...
          if (newImageParent) {
            const newImagePos = Array.from(
              newImageParent.parentElement.children
            ).indexOf(newImageParent);
            const slider = this.container.querySelector(
              selectors$v.productSlideshow
            );
            const isDesktopView = isDesktop();

            if (
              slider &&
              slider.classList.contains(selectors$v.sliderEnabled)
            ) {
  ...
  class QuickAddProduct extends HTMLElement {
    constructor() {
      ...
      if (this.quickAddHolder) {
        ...
        this.isCartItem = Boolean(
          this.quickAddHolder.closest(selectors$u.cartLineItems)
        );
        this.modalTemplate = this.quickAddHolder.querySelector(
          selectors$u.quickAddModalTemplate
        );
        this.modal = document.querySelector(selectors$u.quickAddModal);
        this.modalID = this.quickAddHolder.getAttribute(
          selectors$u.quickAddHolder
        );
        this.modalButton = this.quickAddHolder.querySelector(
          `[data-popup-${this.modalID}]`
        );
        this.handle = this.modalButton
          ? this.modalButton.getAttribute(`data-popup-${this.modalID}`)
          : null;
        this.cartDrawer = document.querySelector(selectors$u.cartDrawer);
        this.buttonQuickAdd = this.quickAddHolder.querySelector(
          selectors$u.buttonQuickAdd
        );
        this.button = this.modalButton || this.buttonQuickAdd;
        this.buttonQuickAddMobile = this.quickAddHolder.querySelector(
          selectors$u.buttonQuickAddMobile
        );
    ...
    init() {
      ...
      if (this.modalButton && this.modalTemplate) {
        this.modalButton.addEventListener("click", (e) => {
          ...
          const isSiblingSwapper = this.modalButton.hasAttribute(
            selectors$u.siblingSwapper
          );
          const isSiblingLinkCurrent = this.modalButton.classList.contains(
            classes$q.siblingLinkCurrent
          );
      ...
      if (this.buttonQuickAdd) {
        const buttonQuickAddText = this.buttonQuickAdd.querySelector(
          selectors$u.buttonQuickAddText
        );

        if (
          buttonQuickAddText &&
          parseInt(
            getComputedStyle(this.buttonQuickAdd).getPropertyValue(
              "--btn-text-width"
            )
          ) === 0
        ) {
          this.buttonQuickAdd.style.setProperty(
            "--btn-text-width",
            `${buttonQuickAddText.clientWidth}px`
          );
        }
        ...
        if (this.buttonQuickAdd.hasAttribute(selectors$u.quickAddVariant)) {
          this.buttonQuickAdd.addEventListener("click", (e) => {
            e.preventDefault();
            const variantID = this.buttonQuickAdd.getAttribute(
              selectors$u.quickAddVariant
            );

            if (variantID) {
              const props = this.buttonQuickAdd.hasAttribute(
                selectors$u.quickAddProp
              )
                ? JSON.parse(
                    this.buttonQuickAdd
                      .getAttribute(selectors$u.quickAddProp)
                      .replaceAll("'", '"')
                  )
                : null;
          ...
          if (
            theme.settings.enableQuickAdd &&
            this.buttonQuickAdd.closest(`[${selectors$u.quickAddHolder}]`)
          ) {
            this.errorHandler();
          }
        }
    ...
    addToCart(id, props = null) {
      const label = this.buttonQuickAdd.closest(selectors$u.quickAddHolder)
        ? this.buttonQuickAdd
        : null;
      let data = {
        id: id,
    ...
    errorHandler() {
      this.quickAddHolder.addEventListener("theme:cart:error", (event) => {
        ...
        const parentProduct = holder.closest(
          `[${selectors$u.productGridItem}]`
        );
        if (!parentProduct) return;
        const errorMessageHolder = holder.querySelector(
          selectors$u.messageError
        );
        const hasOverlayText = parentProduct.classList.contains(
          classes$q.overlayText
        );
        const productInfo = parentProduct.querySelector(
          selectors$u.productInformationHolder
        );
        const button = holder.querySelector(selectors$u.buttonQuickAdd);
        const buttonQuickAddMobile = holder.querySelector(
          selectors$u.buttonQuickAddMobile
        );
        ...
        window
          ...
          .then((response) => {
            ...
            this.modalContent = document.querySelector(
              selectors$u.modalContent
            );
            this.modalContent.innerHTML = fresh.querySelector(
    ...
    initFormFunctionalities() {
      ...
      const swatchElements = this.modalContent.querySelectorAll(
        selectors$u.swatch
      );
      ...
      const wrappers = this.modalContent.querySelectorAll(
        selectors$u.popoutWrapper
      );
      if (wrappers.length) {
        wrappers.forEach((wrapper) => {
          new Popout(wrapper);
    ...
    modalCreate() {
      ...
      MicroModal.show(this.modalID, {
        onShow: (modal, el, event) => {
          ...
          document.dispatchEvent(
            new CustomEvent("theme:quick-add:open", { bubbles: true })
          );
          document.dispatchEvent(
            new CustomEvent("theme:scroll:lock", {
              bubbles: true,
              detail: this.modalContainer,
            })
          );

          this.modalContainer.addEventListener(
            "scroll",
            this.closeTooltipEvent
          );
        },
        onClose: (modal, el, event) => {
          ...
          if (
            this.quickAddHolder &&
            this.quickAddHolder.classList.contains(classes$q.disabled)
          ) {
            this.quickAddHolder.classList.remove(classes$q.disabled);
          }
          ...
          if (
            document.body.classList.contains(classes$q.focused) &&
            this.buttonQuickAdd
          ) {
  ...
  class SiblingSwatches {
    constructor(swatches, product) {
      ...
      this.productLinks = this.product.querySelectorAll(
        selectors$t.productLink
      );
      this.productCutline = this.product.querySelector(
        selectors$t.productCutline
      );
      ...
      this.productImageSibling = this.product.querySelector(
        selectors$t.productImageSibling
      );
    ...
    showSibling(event) {
      ...
      const siblingLink = swatch.hasAttribute(attributes$b.siblingLink)
        ? swatch.getAttribute(attributes$b.siblingLink)
        : "";
      const siblingPrice = swatch.hasAttribute(attributes$b.siblingPrice)
        ? swatch.getAttribute(attributes$b.siblingPrice)
        : "";
      const siblingCutline = swatch.hasAttribute(attributes$b.siblingCutline)
        ? swatch.getAttribute(attributes$b.siblingCutline)
        : "";
      const siblingImage = swatch.hasAttribute(attributes$b.siblingImage)
    ...
    showSiblingImage(siblingImage) {
      ...
      const imageExists = this.productImageSibling.querySelector(
        `[src="${imageSrc}"]`
      );
  ...
  class Sibling {
    constructor(holder, product) {
      this.holder = holder;
      this.product = product;
      this.siblingScrollbar = this.holder.querySelector(
        selectors$t.siblingsInnerHolder
      );
      this.siblingCount = this.holder.querySelector(selectors$t.siblingCount);
      this.siblingFieldset = this.holder.querySelector(
        selectors$t.siblingFieldset
      );
  ...
  class Siblings {
    constructor(section) {
      this.container = section.container;
      this.siblingHolders = this.container.querySelectorAll(
        `${selectors$t.productGridItem} ${selectors$t.siblingHolder}`
      );
      ...
      if (this.siblingHolders.length) {
        this.siblingHolders.forEach((siblingHolder) => {
          new Sibling(
            siblingHolder,
            siblingHolder.closest(selectors$t.productGridItem)
          );
        });
      }
    }
    ...
    init() {
      ...
      if (this.slider.hasAttribute(selectors$s.dataMinValue)) {
        defaultMinValue = parseFloat(
          this.slider.getAttribute(selectors$s.dataMinValue)
        );
      }
      ...
      if (this.slider.hasAttribute(selectors$s.dataMaxValue)) {
        defaultMaxValue = parseFloat(
          this.slider.getAttribute(selectors$s.dataMaxValue)
        );
      }
      ...
      if (this.slider.getAttribute(selectors$s.dataStep)) {
        this.step = Math.abs(
          parseFloat(this.slider.getAttribute(selectors$s.dataStep))
        );
      }
  ...
  class FiltersForm {
    constructor(section) {
      ...
      this.productsContainer = this.container.querySelector(
        selectors$r.productsContainer
      );
      this.collectionNav = this.container.querySelector(
        selectors$r.collectionNav
      );
      this.init();
    }
    ...
    init() {
      if (this.form) {
        ...
        this.sidebar.addEventListener(
          ...
          debounce((e) => {
            ...
            if (
              type === selectors$r.input ||
              type === selectors$r.select ||
              type === selectors$r.label ||
              type === selectors$r.textarea
            ) {
              if (this.form && typeof this.form.submit === "function") {
                ...
                if (priceMin && priceMax) {
                  if (
                    target.hasAttribute(selectors$r.priceMinValue) &&
                    !priceMax.value
                  ) {
                    priceMax.value = priceMax.placeholder;
                  } else if (
                    target.hasAttribute(selectors$r.priceMaxValue) &&
                    !priceMin.value
                  ) {
        ...
        this.sidebar.addEventListener("theme:range:update", (e) =>
          this.updateRange(e)
        );
      }
      ...
      if (this.sort) {
        this.container.addEventListener("theme:filter:update", (e) =>
          this.submitForm(e)
        );
      }
    ...
    filterUpdateFromUrl(e) {
      const target = e.target;
      if (
        target.matches(selectors$r.filterUpdateUrlButton) ||
        (target.closest(selectors$r.filterUpdateUrlButton) && target)
      ) {
        e.preventDefault();
        const button = target.matches(selectors$r.filterUpdateUrlButton)
          ? target
    ...
    submitForm(e, replaceHref = "") {
      ...
      const sortValue = this.sort
        ? this.sort.getAttribute(selectors$r.sort)
        : "";
      if (!e || (e && e.type !== "popstate")) {
        if (replaceHref === "") {
          ...
          if (this.form) {
            ...
            const rangeMinDefaultValue =
              rangeMin && rangeMin.hasAttribute(selectors$r.rangeMinDefault)
                ? rangeMin.getAttribute(selectors$r.rangeMinDefault)
                : "";
            const rangeMaxDefaultValue =
              rangeMax && rangeMax.hasAttribute(selectors$r.rangeMaxDefault)
        ...
        this.sort.dispatchEvent(
          new CustomEvent("theme:filter:sort", { bubbles: false })
        );
      }
        ...
        fetch(`${window.location.pathname}${window.location.search}`)
          ...
          .then((data) => {
            this.productsContainer.innerHTML = new DOMParser()
              ...
              .querySelector(selectors$r.productsContainer).innerHTML;

            if (this.sidebar) {
              this.sidebar.innerHTML = new DOMParser()
                .parseFromString(data, "text/html")
                .querySelector(selectors$r.collectionSidebar).innerHTML;

              const activeFiltersCountContainer = this.sidebar.querySelector(
                `[${selectors$r.activeFiltersCount}]`
              );
              const activeFiltersContainer = this.container.querySelectorAll(
                selectors$r.activeFilters
              );
              if (
                activeFiltersCountContainer &&
                activeFiltersContainer.length
              ) {
                const activeFiltersCount = parseInt(
                  activeFiltersCountContainer.getAttribute(
                    selectors$r.activeFiltersCount
                  )
                );
  ...
  class Collection {
    constructor(section) {
      ...
      this.collectionNav = this.container.querySelector(
        selectors$q.collectionNav
      );
      this.showMoreOptions = this.container.querySelectorAll(
        selectors$q.showMoreOptions
      );
      this.collectionSidebarHeading = this.container.querySelectorAll(
        selectors$q.collectionSidebarHeading
      );
        ...
        this.showMoreOptions.forEach((element) => {
          element.addEventListener("click", (event) => {
            ...
            element.parentElement.previousElementSibling
              .querySelectorAll(selectors$q.linkHidden)
              .forEach((link, index) => {
        ...
        this.collectionSidebarHeading.forEach((element) => {
          ...
          element.addEventListener("keyup", (event) => {
            if (
              (event.code === window.theme.keyboardKeys.SPACE ||
                event.code === window.theme.keyboardKeys.ENTER) &&
              document.body.classList.contains(classes$m.focused)
            ) {
    ...
    collectionAccordion(event) {
      ...
      if (collectionSidebarSlideOut) {
        const removeTrapFocusOnSlideOut = () => {
          ...
          collectionSidebarSlideOut.removeEventListener(
            "transitionend",
            removeTrapFocusOnSlideOut
          );
        };

        collectionSidebarSlideOut.addEventListener(
          "transitionend",
          removeTrapFocusOnSlideOut
        );
      }
    ...
    sortActions(link, submitForm = true) {
      ...
      const sortButtonText = this.sort.querySelector(
        selectors$q.sortButtonText
      );
    ...
    onSortCheck(e) {
      ...
      if (window.location.search.includes("sort_by")) {
        ...
        for (const [key, val] of urlParams.entries()) {
          const linkSort = this.sort.querySelector(
            `[${selectors$q.sortValue}="${val}"]`
          );
    ...
    initSort() {
      ...
      if (this.sortButton) {
        this.sortButton.addEventListener("click", () => {
          const isFiltersSidebarOpen =
            this.collectionSidebar.classList.contains(classes$m.expanded);
    ...
    showSidebarCallback() {
      const collectionSidebarSlider = this.container.querySelector(
        selectors$q.collectionSidebarSlider
      );
      const collectionSidebarSlideOut = this.container.querySelector(
        selectors$q.collectionSidebarSlideOut
      );
      const collectionSidebarScrollable =
        collectionSidebarSlider || collectionSidebarSlideOut;
      ...
      if (isMobileView || collectionSidebarSlideOut !== null) {
        ...
        document.dispatchEvent(
          new CustomEvent("theme:scroll:lock", {
            bubbles: true,
            detail: collectionSidebarScrollable,
          })
        );
      }

      if (collectionSidebarScrollable) {
        collectionSidebarScrollable.addEventListener(
          "scroll",
          this.collectionSidebarScrollEvent
        );
      }
    }
    ...
    hideSidebar() {
      ...
      if (collectionSidebarScrollable) {
        collectionSidebarScrollable.removeEventListener(
          "scroll",
          this.collectionSidebarScrollEvent
        );
      }
    ...
    onUnload(flag = true) {
      ...
      if (this.collectionSidebarScrollable & flag) {
        this.collectionSidebarScrollable.removeEventListener(
          "scroll",
          this.collectionSidebarScrollEvent
        );
      }
      ...
      if (this.underlay) {
        this.underlay.removeEventListener(
          "click",
          this.collectionSidebarCloseEvent
        );
      }
  ...
  class Ticker {
    ...
    checkWidth() {
      ...
      const speed = this.frame.getAttribute(attributes$a.speed)
        ? this.frame.getAttribute(attributes$a.speed)
        : settings.textAnimationTime;
      ...
      if (
        ...
        if (this.scale.childElementCount === 1) {
          ...
          const animationTimeFrame = (
            (this.text.clientWidth / settings.space) *
            Number(speed)
          ).toFixed(2);
  ...
  class Bar {
    ...
    init() {
      ...
      document.dispatchEvent(
        new CustomEvent("theme:announcement:init", { bubbles: true })
      );
    }
      ...
      for (let index = 0; index < this.slides.length; index++) {
        ...
        if (
          this.locationPath.indexOf(
            element.getAttribute(attributes$9.targetReferrer)
          ) === -1 &&
    ...
    toggleTicker(e, isStopped) {
      ...
      const element = document.querySelector(
        `[${attributes$9.slide}="${e.detail.blockId}"]`
      );
  ...
  class Drawer {
    constructor(el) {
      ...
      this.scrollableElements = this.drawer.querySelectorAll(
        selectors$n.scrollableElements
      );
    ...
    hideDrawer() {
      ...
      this.accessibility.removeTrapFocus();
      if (
        this.body.classList.contains(classes$j.isFocused) &&
        this.buttons.length
      ) {
        this.buttons[0].focus();
      }
  ...
  class HoverDisclosure {
    constructor(el) {
      ...
      this.trigger = document.querySelector(
        `[${selectors$l.disclosureToggle}='${this.key}']`
      );
    ...
    hideDisclosure() {
      ...
      this.wrapper.classList.remove(
        classes$h.meganavVisible,
        classes$h.meganavIsTransitioning
      );
    }
    ...
    staggerChildAnimations() {
      ...
      pairs.forEach((child, i) => {
        ...
        child.parentElement
          .querySelectorAll(selectors$l.staggerAfter)
  ...
  const hoverDisclosure = {
    onLoad() {
      ...
      disclosures = this.container.querySelectorAll(
        selectors$l.disclosureWrappper
      );
    ...
    update() {
      if (this.cart) {
        this.counts.forEach((count) => {
          ...
          count.innerHTML =
            this.cart.item_count < 10 ? `${this.cart.item_count}` : "9+";
        });
      }
    }
  }
  ...
  class HeaderMobileSliderule {
    ...
    trapFocusSliderule(showSliderule = true) {
      ...
      if (trapFocusButton && this.drawerContent) {
        this.accessibility.trapFocus(this.drawerContent, {
          elementToFocus: document.body.classList.contains(classes$g.focused)
            ? trapFocusButton
            : null,
        });
      }
    ...
    hideSliderule(close = false) {
      ...
      const hiddenSelector = close
        ? `[${selectors$j.dataAnimates}].${classes$g.isHidden}`
        : `[${selectors$j.dataAnimates}="${newPosition}"]`;
      ...
      const children = close
        ? this.pane.querySelectorAll(
            `.${classes$g.isVisible}, .${classes$g.isHiding}`
          )
        : this.children;
      children.forEach((element, index) => {
        ...
        const removeHidingClass = () => {
          if (
            parseInt(this.pane.getAttribute(selectors$j.sliderulePane)) ===
            newPosition
          ) {
    ...
    showSliderule() {
      ...
      const hiddenItems = this.pane.querySelectorAll(
        `[${selectors$j.dataAnimates}="${oldPosition}"]`
      );
      if (hiddenItems.length) {
    ...
    closeSliderule() {
      if (
        this.pane &&
        this.pane.hasAttribute(selectors$j.sliderulePane) &&
        parseInt(this.pane.getAttribute(selectors$j.sliderulePane)) > 0
      ) {
  ...
  class Header {
    constructor(el) {
      ...
      this.isTransparentHeader =
        this.wrapper.getAttribute(selectors$i.transparent) !== "false";
      this.overlayedImages = document.querySelectorAll(
        selectors$i.firstSectionHasImage
      );
    ...
    checkForImage() {
      ...
      let preventTransparentHeader = document.querySelectorAll(
        selectors$i.preventTransparentHeader
      ).length;
    ...
    getMinWidth() {
      ...
      const widthWrappers = comparitor.querySelectorAll(
        selectors$i.widthContentWrapper
      );
      let minWidth = 0;
      let spaced = 0;
  ...
  class BlockScroll {
    ...
    onBlockSelect(evt) {
      const scrollElement = this.container.querySelector(
        selectors$h.scrollElement
      );
      if (
        scrollElement &&
        !scrollElement.classList.contains(selectors$h.flickityEnabled)
      ) {
  ...
  class Look {
    constructor(section) {
      ...
      this.popupContainer = this.container.querySelector(
        selectors$g.popupContainer
      );
      this.popupContent = this.container.querySelector(
        selectors$g.popupContent
      );
      ...
      if (this.slider && this.slides.length && this.thumbs.length) {
        ...
        this.thumbs.forEach((thumb, i) => {
          thumb.addEventListener("click", (e) => {
            e.preventDefault();
            const idx =
              thumb.hasAttribute(attributes$7.thumbValue) &&
              thumb.getAttribute(attributes$7.thumbValue) !== ""
                ? parseInt(thumb.getAttribute(attributes$7.thumbValue))
                : i;
            ...
            if (isMobile()) {
              ...
              const headerHeight =
                ...
                document.querySelector(selectors$g.headerHeight)
                  ? document
                      .querySelector(selectors$g.headerHeight)
                      .getBoundingClientRect().height
                  : 0;
              ...
              const sliderContainer = this.container.querySelector(
                selectors$g.sliderContainer
              );
              let scrollTarget =
  ...
  class GlobalTabs {
    ...
    init() {
      ...
      const startIdx = ctx.hasAttribute(selectors$f.dataTabStartIndex)
        ? ctx.getAttribute(selectors$f.dataTabStartIndex)
        : 0;
      let firstTabLink = ctx.querySelector(
        `${selectors$f.tabLink}-${startIdx}`
      );
      let firstTabContent = ctx.querySelector(
        `${selectors$f.tabContent}-${startIdx}`
      );

      if (isMobile()) {
        const startIdxMobile = ctx.hasAttribute(
          selectors$f.dataTabStartIndexMobile
        )
          ? ctx.getAttribute(selectors$f.dataTabStartIndexMobile)
          : startIdx;
        firstTabLink = ctx.querySelector(
          `${selectors$f.tabLink}-${startIdxMobile}`
        );
        firstTabContent = ctx.querySelector(
          `${selectors$f.tabContent}-${startIdxMobile}`
        );
      }
        ...
        tabsNavList.forEach((element) => {
          ...
          element.addEventListener("keyup", (event) => {
            if (
              (event.code === window.theme.keyboardKeys.SPACE ||
                event.code === window.theme.keyboardKeys.ENTER) &&
              this.body.classList.contains(classes$d.focused)
            ) {
              this.tabChange(element, tab);
            }
          });
    ...
    tabChange(element, tab) {
      ...
      this.container
        .querySelector(`${selectors$f.tabsLi}.${classes$d.current}`)
        .classList.remove(classes$d.current);
      const lastCurrentTab = this.container.querySelector(
        `${selectors$f.tabContent}.${classes$d.current}`
      );
      ...
      this.container.dispatchEvent(
        new CustomEvent("theme:tab:change", { bubbles: true })
      );
    ...
    checkVisibleTabLinks() {
      ...
      const tabsNavListHidden = this.container.querySelectorAll(
        `${selectors$f.tabLink}.${classes$d.hidden}`
      );
  ...
  class PickupAvailability {
    ...
    fetchPickupAvailability(event) {
      ...
      const variantID =
        event && event.detail.variant
          ...
          : container.getAttribute(selectors$e.pickupContainer);
        ...
        fetch(
          ...
          .then((text) => {
            const pickupAvailabilityHTML = new DOMParser()
              .parseFromString(text, "text/html")
              .querySelector(selectors$e.shopifySection).innerHTML;
            ...
            this.buttonDrawerOpen = this.body.querySelector(
              selectors$e.drawerOpen
            );
    ...
    openDrawer() {
      if (this.drawer) {
        ...
        setTimeout(() => {
          const elementToFocus = this.drawer.querySelector(
            selectors$e.drawerClose
          );
          this.a11y.removeTrapFocus();
    ...
    closeDrawer() {
      if (this.drawer) {
        ...
        if (
          window.accessibility.lastElement &&
          this.body.classList.contains(classes$c.isFocused)
        ) {
          requestAnimationFrame(() => {
            window.accessibility.lastElement.focus();
          });
    ...
    closeEvent() {
      document.addEventListener("click", (event) => {
        const clickedElement = event.target;
        const isNotDrawerUnderlay = !(
          clickedElement.matches(selectors$e.drawerUnderlay) ||
          clickedElement.closest(selectors$e.drawerUnderlay)
        );
        const isNotDrawerOrDrawerChild =
          !(
            clickedElement.matches(selectors$e.drawerOpen) ||
            clickedElement.closest(selectors$e.drawer)
          ) || !isNotDrawerUnderlay;

        if (
          isNotDrawerOrDrawerChild &&
          this.drawer?.classList.contains(classes$c.isOpen)
        ) {
          this.closeDrawer();
        }
      });
  ...
  class ComplementaryProducts extends HTMLElement {
    ...
    connectedCallback() {
      const handleIntersection = (entries, observer) => {
        ...
        if (
          this.hasAttribute(attributes$6.url) &&
          this.getAttribute(attributes$6.url) !== ""
        ) {
          fetch(this.getAttribute(attributes$6.url))
            ...
            .then((text) => {
              ...
              const recommendations = html.querySelector(
                selectors$d.complementaryProducts
              );
              ...
              if (
                html.querySelector(
                  `${selectors$d.complementaryProducts} ${selectors$d.quickAddProduct}`
                )
              ) {
  ...
  class ProductSticky {
    constructor(section) {
      ...
      this.container = section.container;
      this.stickyEnabled =
        this.container.getAttribute(selectors$c.dataStickyEnabled) === "true";
    ...
    stickyScrollCheck() {
      const targetFormWrapper = this.container.querySelector(
        `${selectors$c.productPage} ${selectors$c.formWrapper}`
      );
  ...
  class Product {
    constructor(section) {
      ...
      this.productPopupButton = this.container.querySelectorAll(
        `[${selectors$a.productPopupButton}]`
      );
      this.formWrapper = this.container.querySelector(selectors$a.formWrapper);
      this.cartBarExist =
        this.container.getAttribute(selectors$a.dataCartBar) === "true";
      ...
      this.productPopupButton.forEach((button) => {
        button.addEventListener("click", (e) => {
          ...
          const modal = document.querySelector(
            `#${button.getAttribute(selectors$a.productPopupButton)}`
          );
          const modalScrollContainer = modal.querySelector(
            selectors$a.modalScrollContainer
          );
    ...
    truncateText() {
      if (this.truncateElementHolder.classList.contains(classes$8.visible))
        return;
    ...
    toggleTruncatedContent(holder) {
      const toggleButton = holder.querySelector(
        selectors$a.toggleTruncateButton
      );
      if (toggleButton) {
    ...
    cartBarScroll() {
      ...
      if (element && this.cartBar) {
        ...
        if (siteFooter) {
          ...
          siteFooter.style.marginBottom = siteFooter.classList.contains(
            classes$8.siteFooterPush
          )
  ...
  class Related {
    ...
    related() {
      ...
      const relatedProducts = relatedSection.querySelectorAll(
      ...
      fetch(requestUrl)
        ...
        .then((data) => {
          ...
          const hasProducts = relatedContent.querySelector(
            selectors$9.relatedProduct
          );
    ...
    recent() {
      ...
      Shopify.Products.showRecentlyViewed({
        ...
        onComplete: (wrapper, section) => {
          ...
          const recentProducts = wrapper.querySelectorAll(
            selectors$9.productItem
          );
  ...
  class LogoList {
    ...
    sliderAccessibility() {
      ...
      if (buttons.length) {
        buttons.forEach((button) => {
          ...
          if (slide) {
            const tabIndex = slide.classList.contains(classes$6.isSelected)
              ? 0
              : -1;
            button.setAttribute("tabindex", tabIndex);
          }
      ...
      if (this.logoSlides.length) {
        this.logoSlides.forEach((logoItem) => {
          logoItem.addEventListener("click", (e) => {
            ...
            const hasSlider = this.slideshowNav.classList.contains(
              classes$6.flickityEnabled
            );
    ...
    onBlockDeselect() {
      if (
        this.slideshowNav &&
        this.slideshowNav.classList.contains(classes$6.flickityEnabled)
      ) {
        this.flktyNav.playPlayer();
  ...
  class CompareImages {
    ...
    changeValues(event) {
      ...
      if (
        !event ||
        (event && event.type !== "touchmove" && event.type !== "mousemove")
      ) {
        this.imageElement.style.width = `${imagesHolderWidth}px`;
        this.imageHolder.style.width = `${100 - parseInt(this.startPosition)}%`;

        if (this.startPosition !== 0) {
          const newButtonPositionPixels =
            (imagesHolderWidth * parseInt(this.startPosition)) / 100;
  ...
  class TargetReferrer {
    ...
    init() {
      if (
        this.locationPath.indexOf(
          this.el.getAttribute(selectors$4.dataTargetReferrer)
        ) === -1 &&
        !window.Shopify.designMode
      ) {
  ...
  class LargePopup {
    constructor(el) {
      ...
      this.cookie = new PopupCookie(
        this.popup.getAttribute(selectors$4.cookieNameAttribute),
        "user_has_closed"
      );
  ...
  class Password {
    constructor(section) {
      this.container = section.container;
      this.passwordLogin = this.container.querySelectorAll(
        selectors$3.passwordLogin
      );
  ...
  class ScrollToElement {
      ...
      this.scrollToButtons.forEach((element) => {
        element.addEventListener("click", () => {
          const target = this.container.querySelector(
            element.getAttribute(attributes$2.scrollToElementValue)
          );

          if (!target || element.tagName === "A") return;

          const accordionToggle = target.querySelector(
            selectors$2.accordionToggle
          );
          let timeoutFlag = false;

          // Open target accordion if they are inside it
          if (accordionToggle) {
            const accordionHolder = accordionToggle.closest(
              selectors$2.accordionHolder
            );

            if (
              !accordionToggle.classList.contains(classes$2.open) &&
              accordionHolder &&
              accordionHolder.querySelector(
                `${selectors$2.accordionToggle}.${classes$2.open}`
              )
            ) {
              timeoutFlag = true;
            }
    ...
    scrollToElement(element) {
      ...
      const tooltips = document.querySelectorAll(
        `${selectors$2.tooltip}:not([${attributes$2.tooltipStopMousenterValue}])`
      );
      if (tooltips.length) {
        tooltips.forEach((tooltip) => {
  ...
  class ScrollSpy {
    constructor(section) {
      this.section = section;
      this.container = section.container;
      this.scrollSpyAnchors = this.container.querySelectorAll(
        selectors$1.scrollSpy
      );
    ...
    toggleObserver(anchor) {
      const anchorSpy = this.container.querySelector(
        anchor.getAttribute(attributes$1.scrollSpy)
      );
      ...
      const isEligible =
        ...
        (!anchor.hasAttribute(attributes$1.desktop) &&
          !anchor.hasAttribute(attributes$1.mobile));
    ...
    runObserver(anchorSpy) {
      ...
      const stickyHeader = Boolean(
        document.querySelector(selectors$1.headerSticky)
      );
      const headerHeight = stickyHeader ? menuHeight : 0;
      const rootMargin = stickyHeader
        ? headerHeight * -1 + "px 0px 0px 0px"
        : "0px";
      ...
      this.observers[anchorSpy.id] = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          const anchorOld = this.container.querySelector(
            `[${attributes$1.scrollSpy}].${classes$1.selected}`
          );
          const anchorNew = this.container.querySelector(
            `[${attributes$1.scrollSpy}="#${entry.target.id}"]`
          );
  ...
  class CollectionsHover {
    ...
    updateState(targetId) {
      const button = this.container.querySelector(
        `[${attributes.target}="${targetId}"]`
      );
      const target = this.container.querySelector(
        `#${targetId}:not(.${classes.visible})`
      );
      const buttonSelected = this.container.querySelector(
        `${selectors.button}.${classes.selected}`
      );
      const imageVisible = this.container.querySelector(
        `${selectors.image}.${classes.visible}`
      );
...
const portfolio = screen.children[0];

portfolio.addEventListener("mouseover", easeScroll);

function easeScroll() {
  portfolio.removeEventListener("mouseover", easeScroll);

  let currentScroll = screen.scrollTop;

  if (currentScroll < 5000) {
    window.requestAnimationFrame(easeScroll);
    screen.scrollTo(0, currentScroll + 7);
  }
}
